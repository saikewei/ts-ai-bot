...
public class WakeWordRuntime : IDisposable
{
    ...
    public WakeWordRuntime(WakeWordRuntimeConfig settings)
    {
        ...
        // 拼接 Mel 和 Emb 模型的绝对路径
        var absoluteMelPath = Path.Combine(baseDir, MelModelPath);
        var absoluteEmbPath = Path.Combine(baseDir, EmbModelPath);

        if (!File.Exists(absoluteMelPath)) throw new InvalidOperationException($"Model file {absoluteMelPath} does not exist.");
        if (!File.Exists(absoluteEmbPath)) throw new InvalidOperationException($"Model file {absoluteEmbPath} does not exist.");

        // Load ONNX models
        _melSession = new InferenceSession(absoluteMelPath);
        _embSession = new InferenceSession(absoluteEmbPath);

        _wwSessions = new List<InferenceSession>();
        foreach (var ww in _settings.WakeWords)
        {
            var modelPath = Path.Combine(baseDir,  "models", $"{ww.Model}.onnx");
